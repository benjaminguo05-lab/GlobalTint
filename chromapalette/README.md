...
## 0.1.8：修复 #13 的 Filza 箭头回归

用户确认 #12 的箭头有效，#13 再次显示蓝色。两版对 Filza 的主要行为差异是改为只安装一次图片工厂钩子；真机上是否存在晚期覆盖尚未确认。本次不依赖重复安装或只在图片生成时改色：

- 为 Filza 4.0.1-4 已确认的 `ShortenSortButton.sortLabels` 中的 `imageView` 和 `ButtonsGroup.arrowImageView` 增加直接适配。
- 在布局、显示、配置更新以及 `updateLabels` / `updateSubviews` / `themeUpdate` 后补上颜色，覆盖生成早于插件初始化的缓存箭头。
- 只对当前排序列的箭头使用通用强调色，未选中列的 `no_sort` 灰色图标保持原色。
- `arrow_up`、`arrow_down`、`e_return`、`e_expand` 由视图适配负责，图片工厂保留这四个源图，方便关闭改色时恢复。其他图片工厂适配保留。
- 添加缓存箭头出现较早、重复刷新、更换颜色、关闭恢复、切换排序后保留灰色图标的回归用例。仍使用原有更新限额，不重复安装钩子。

安装后彻底关闭并重开 Filza，检查排序箭头和折叠箭头；点击“名称／日期”切换排序，确认只有当前列箭头改色。关闭通用强调色后重开，检查恢复蓝色。编译和回归测试不能替代真机确认。
...
下载本次 Actions 的 `ChromaPalette-RootHide-运行编号`，解压安装版本为 **0.1.8** 的 `.deb`，按提示重新载入桌面并重开照片和 App Store。
